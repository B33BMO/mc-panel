...
export function startServer(name: string): { ok: boolean; message: string } {
  const dir = paths.server(name);
  if (!fs.existsSync(dir)) return { ok:false, message:"Server folder missing" };
  const pid = readPid(name);
  if (isRunning(pid)) return { ok:false, message:"Already running" };
  ...
  if (process.platform === "win32" && fs.existsSync(paths.startBat(name))) {
    cmd = "cmd"; args = ["/c", "start", "/min", "start.bat"]; shell = true;
  } else if (fs.existsSync(paths.startSh(name))) {
    cmd = "bash"; args = ["start.sh"]; 
  } else if (fs.existsSync(paths.jar(name))) {
    cmd = JAVA; args = ["-Xmx4G", "-jar", "server.jar", "nogui"];
  }
  ...
  child.unref();
  fs.writeFileSync(paths.pid(name), String(child.pid || ""), "utf8");
  return { ok:true, message:`Launched (pid ${child.pid ?? "?"})` };
}
...
export function restartServer(name: string) {
  stopServer(name); return startServer(name);
}
...
export async function stats(name: string) {
  ...
  if (running) {
    ...
    cpu = p.cpu; memMB = (p.memory ?? 0) / (1024*1024);
  }
  ...
  try {
    const out = execSync("nvidia-smi --query-gpu=memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits", { stdio:["ignore","pipe","ignore"] }).toString().trim();
    const [used, total, util] = out.split(",").map(s => parseFloat(s.trim()));
    gpu = { usedMB: used, totalMB: total, pct: util };
  } catch { /* no nvidia-smi */ }

  const totalMB = undefined; // could read from system if you want
